#package handling moved to main scripts starting 6/13/2024

# Function to fetch rain site codes that also have air temperature
fetch_rain_ATC_site_codes <- function(con) {
  query <- "SELECT DISTINCT SITE_CODE 
              FROM tblGaugeLLID
              WHERE status = 'Active' AND Precip = 1 AND AirTemp = 1"
  
  rain_ATC_site_codes <- dbGetQuery(con, query)$SITE_CODE
  return(rain_ATC_site_codes)
}

# Function to fetch water temperature site codes
fetch_WTC_site_codes <- function(con) {
  query <- "SELECT DISTINCT SITE_CODE 
              FROM tblGaugeLLID 
              WHERE status = 'Active' and WaterTemp = 1"
  
  WTC_site_codes <- dbGetQuery(con, query)$SITE_CODE
  return(WTC_site_codes)
}

# Function to fetch rain site codes that also have air temperature
fetch_rain_site_codes <- function(con) {
  query <- "SELECT DISTINCT SITE_CODE 
              FROM tblGaugeLLID
              WHERE status = 'Active' AND Precip = 1"
  
  rain_site_codes <- dbGetQuery(con, query)$SITE_CODE
  return(rain_site_codes)
}

# Function to fetch discharge site codes
fetch_discharge_site_codes <- function(con) {
  query <- "SELECT DISTINCT SITE_CODE 
              FROM tblGaugeLLID 
              WHERE status = 'Active' and FlowLevel = 1"
  
  discharge_site_codes <- dbGetQuery(con, query)$SITE_CODE
  return(discharge_site_codes)
}
...
retrieve_WTC_data <- function(con, start_date, end_date, WTC_site_codes) {
  ...
  for (WTC_site_code in WTC_site_codes) {
    ...
    if (nrow(result) > 0) {
      ...
      # Fill missing timestamps with the last available W_Value
...
# Function to retrieve field water temperature data
...
# Function to retrieve field discharge data
retrieve_field_discharge <- function(con, start_date, end_date, discharge_site_codes) {
  ...
  for (discharge_site_code in discharge_site_codes) {
    ...
    if (nrow(result) > 0) {
      ...
      cat("Warning: Field Discharge data is not available for site_code =", discharge_site_code, "\n")
    }
  }
...
for (water_level_site_code in water_level_site_codes) {
    result <- dbGetQuery(con, query, params = list(start_date, end_date, water_level_site_code))
    
    if (nrow(result) > 0) {
      data_list[[water_level_site_code]] <- result
    } else {
      cat("Warning: Lake level data is not available for site_code =", water_level_site_code, "\n")
    }
  }
...
retrieve_instrument_data <- function(con, start_date, end_date, discharge_site_codes) {
  data_list <- list()
    ...
    if (nrow(result) > 0) {
      ...
      cat("Warning: Instrument data is not available for site_code =", discharge_site_code, "\n")
    }
  }
...
retrieve_field_WL <- function(con, start_date, end_date, discharge_site_codes) {
  data_list <- list()  # Create a list to store the data for each combination
    ...
    if (nrow(result) > 0) {
      ...
      cat("Warning: Field water level data is not available for site_code =", discharge_site_code, "\n")
    }
  }
...
for (well_level_site_code in well_level_site_codes) {
  result <- dbGetQuery(con, query, params = list(start_date, end_date, well_level_site_code))
    
    if (nrow(result) > 0) {
      data_list[[well_level_site_code]] <- result
    } else {
      cat("Warning: Well level data is not available for site_code =", well_level_site_code, "\n")
    }
  }
...
filter_data <- function(data, start_date, end_date, discharge_site_codes) {
  ...
  for (discharge_site_code in discharge_site_codes) {
    ...
    if (nrow(filtered_data) > 0) {
      data_list[[discharge_site_code]] <- filtered_data
    } else {
      cat("Warning: Data is not available for site_code =", discharge_site_code, "\n")
    }
  }
